      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        if (text.includes('pageData')) {
          ...
          if (!commitMsg) {
            ...
            commitMsg = `Time: ${resultRuntime}, Memory: ${resultMemory} - AlgoSync`;
          }
...
/* Creates a ✔️ tick mark before "Run Code" button signaling AlgoSync has done its job */
...
LeetCodeV1.prototype.checkSubmissionSucceeded = async function (_iterations) {
  return this.getSuccessStateAndUpdate();
};
LeetCodeV1.prototype.getSubmissionDetails = async function () {
...
LeetCodeV2.prototype.checkSubmissionSucceeded = async function (_iterations) {
  const successTag = document.querySelectorAll('[data-e2e-locator="submission-result"]');
  if (checkElem(successTag)) {
    const text = successTag[0].innerText || '';
  ...
  if (match) {
    this.submissionId = match[1];
  }
  return this.getSuccessStateAndUpdate();
};
LeetCodeV2.prototype.getLatestSubmissionId = async function () {
  if (this.submissionId) return this.submissionId;
  const match = window.location.href.match(/submissions\/(?:detail\/)?(\d+)/);
  if (match) {
    this.submissionId = match[1];
  ...
  try {
    const titleSlugMatch = window.location.pathname.match(/\/problems\/([^/]+)/);
    if (titleSlugMatch) {
  ...
  questionSubmissionList(offset: $offset, limit: $limit, questionSlug: $questionSlug) {
    submissions { id statusDisplay }
      ...
      const subs = res?.data?.questionSubmissionList?.submissions;
      if (subs && subs.length > 0 && subs[0].statusDisplay === 'Accepted') {
        this.submissionId = subs[0].id;
        return this.submissionId;
      }
    }
  } catch (_e) {}
...
LeetCodeV2.prototype.getSubmissionDetails = async function () {
  await this.getLatestSubmissionId();
  if (this.submissionId && !this.submissionData) {
    await this.init();
  }
  return this.parseStats();
};
