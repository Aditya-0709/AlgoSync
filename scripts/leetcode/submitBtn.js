...
function addManualSubmitBtn(eventHandler) {
  ...
  if (btns.innerText.includes('Solution') && !btns.innerText.includes('AlgoSync') && !btns.innerText.includes('LeetHub')) {
    btns.appendChild(
      (() => {
        const btn = document.createElement('button');
        btn.innerText = 'Sync w/ AlgoSync';
...
function setupManualSubmitBtn(submitBtnHandler) {
  ...
  const pageObserver = new MutationObserver((_, observer) => {
    ...
    if (window.location.href.match(/leetcode\.com\/(.*)\/submissions\/(?:detail\/)?(\d+)/)) {
